...
@Injectable()
export class ServicesAnalysisParser extends ParserService {
  parse(currentWeekElasticData: any, lastWeekElasticData?: any) {
    const parseBuckets = (elasticData: any) => {
      return (
        elasticData?.aggregations?.stats_by_service?.buckets?.map(
          (bucket: any) => {
            ...
            const volume = +(
              bucket.doc_count_per_service?.value?.toFixed(2) ||
              bucket.doc_count ||
              '0'
            );
